...
export interface AudioSink {
  sampleRate: number;
  init(): void;
  onEvent(evt: VGEvent, t: number): void;
  render(duration: number): Float32Array;
}
...
export class OfflineAudioSink implements AudioSink {
  sampleRate = 44100;
  ...
  private pluck(startSec: number, freq: number, duration: number, amp: number): void {
  ...
  private addNoise(startSec: number, duration: number, amp: number, seedText: string): void {
    const start = Math.max(0, Math.floor(startSec * this.sampleRate));
    const length = Math.floor(duration * this.sampleRate);
    const end = Math.min(this.buffer.length, start + length);
